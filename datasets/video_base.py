...
class RGBXYZDataset(VideoDataset):
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        record = self.records[idx]

        # Use deterministic seed so that RGB and XYZ get the same spatial transform
        seed = (idx + 1) * 12345
        random.seed(seed)
        torch.manual_seed(seed)

        # Load rgb data - either raw or preprocessed latents
        rgb = self._load_rgb(record)
        xyz = self._load_xyz(record)
        # images = rgb[:1].clone() if self.image_to_video else None
        image_latents, video_latents = None, None
        rgb_metadata = {
        ...
        if self.load_video_latent:
            ...
            assert rgb_metadata["num_frames"] == n_frames, "num_frames changed"
            assert rgb_metadata["height"] == height, "height changed"
            assert rgb_metadata["width"] == width, "width changed"
        ...
        if "caption" in record:
            ...
            caption = record["original_caption"]
        rgb_metadata["has_caption"] = caption != ""
        prompts = self.id_token + caption
        prompt_embeds = None
        prompt_embed_len = None
        if self.load_prompt_embed:
        ...
        output = {
            ...
            "bbox_render": bbox_render,
            "has_bbox": has_bbox,
            "rgb_path": str(self.data_root / record["rgb_path"]),
        }
    ...
    def _load_rgb(self, record: Dict[str, Any]) -> torch.Tensor:
        """
        Given a record, return a tensor of shape (n_frames, 3, H, W)
        """

        rgb_path = self.data_root / record["rgb_path"]
        rgb_reader = decord.VideoReader(uri=rgb_path.as_posix())
        n_frames = len(rgb_reader)
        start = record.get("trim_start", 0)
        end = record.get("trim_end", n_frames)
        indices = self._temporal_sample(end - start, record["fps"])
        indices = list(start + indices)
        frames = rgb_reader.get_batch(indices)
    ...
    def _load_xyz(self, record: Dict[str, Any]) -> torch.Tensor:
        """
        Load XYZ from npy file with same temporal sampling and spatial transform as rgb.
        Returns a tensor of shape (n_frames, 3, H, W).

        Expected XYZ file:
            xyz.npz contains:
                - xyz: (T, H, W, 3) normalized xyz
                - center: (3,)
                - scale: (1,)
        """
        xyz_path = record.get("xyz_path")
        xyz_path = self.data_root / xyz_path

        frames = np.load(xyz_path)["xyz"]
        # Some xyz dumps contain NaN/Inf; sanitize to avoid propagating NaNs into VAE/model.
        frames = np.nan_to_num(frames, nan=0.0, posinf=1.0, neginf=-1.0)
        frames = np.clip(frames, -1.0, 1.0).transpose(0, 3, 1, 2)

        n_frames_src = frames.shape[0]
        start = record.get("trim_start", 0)
        end = record.get("trim_end", n_frames_src)
        indices = self._temporal_sample(end - start, record["fps"])
        indices = list(start + np.clip(indices, 0, n_frames_src - 1))
        frames = frames[indices]

        if len(frames) != self.n_frames:
            raise ValueError(
                f"Expected {len(frames)=} to be equal to {self.n_frames=} after temporal sample."
            )
        ...
        frames = torch.from_numpy(frames).float().contiguous()
